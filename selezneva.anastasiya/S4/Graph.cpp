...
const std::string ERROR_ONLY_DIRECTED = "Топологическая сортировка только для ориентированных графов";
...
std::ostream & operator<<(std::ostream & os, const Graph & graph) {
    ...
    if (sentry) {
        ...
        int i = 0;
        std::for_each(graph.adjacencyList_.begin(), graph.adjacencyList_.end(), [&](const auto& list) {
            os << "  " << i++ << ":";
...
void Graph::dfsVisit(int vertex, std::vector<int>& colors, std::vector<int>& result) {
    ...
    std::for_each(adjacencyList_[vertex].begin(), adjacencyList_[vertex].end(), [&](const Edge& e) {
...
std::vector<int> Graph::topologicalSort() {
    if (!oriented_) {
        throw std::runtime_error(ERROR_ONLY_DIRECTED);
    }
    std::vector<int> colors(vertexCount_, WHITE);
...
void Graph::topoVisit(int vertex, std::vector<int>& colors, std::vector<int>& stack) {
    ...
    std::for_each(adjacencyList_[vertex].begin(), adjacencyList_[vertex].end(), processNeighbor);
